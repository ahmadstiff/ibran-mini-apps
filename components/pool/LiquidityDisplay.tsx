...
export const LiquidityDisplay: React.FC<LiquidityDisplayProps> = ({
  ...
  const tokenDecimals = React.useMemo(() => {
    const token = tokens.find(
      (t) => t.addresses[chainId]?.toLowerCase() === borrowTokenAddress.toLowerCase()
    );
  ...
  return <div className="text-green-400 font-medium text-center">{totalSupplyAssetsFormatted}</div>;
};
