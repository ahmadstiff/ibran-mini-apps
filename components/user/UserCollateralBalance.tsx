...
export const UserCollateralBalance: React.FC<UserCollateralBalanceProps> = ({
  ...
  const tokenDecimals = React.useMemo(() => {
    const token = tokens.find(
      (t) => t.addresses[chainId]?.toLowerCase() === collateralTokenAddress.toLowerCase()
    );
    return token?.decimals || 18;
  }, [collateralTokenAddress, chainId]);

  const { userCollateralFormatted, userCollateralParsed, collateralLoading, collateralError } =
  ...
  if (collateralError) {
    return <div className={`text-red-400 text-sm ${className}`}>Error loading collateral</div>;
  }

  return (
    <div className={className}>
      {showLabel && (
        <span className="text-gray-400 text-sm font-medium mr-2">Your Collateral:</span>
      )}
