...
export const UserBorrowBalanceDisplay: React.FC<UserBorrowBalanceDisplayProps> = ({
  ...
  } = useReadUserBorrowShares(market.id as `0x${string}`, tokenDecimals);
  ...
  } = useReadTotalSupplyAssets(market.id as `0x${string}`, tokenDecimals);

  // Get max user borrow amount from contract
  const { maxUserBorrow, isLoadingMaxUserBorrow, refetchMaxUserBorrow } = useReadMaxUserBorrow(
