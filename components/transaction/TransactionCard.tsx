...
export const TransactionCard: React.FC<TransactionCardProps> = ({ transaction }) => {
  ...
  const getTransactionIcon = (type: string) => {
    switch (type) {
      ...
      case "supply_collateral":
        return <ArrowDownLeft className="w-5 h-5 text-green-400 drop-shadow-lg" />;
      case "withdraw_liquidity":
        return <ArrowUpRight className="w-5 h-5 text-blue-400 drop-shadow-lg" />;
      case "borrow_debt":
        return <ArrowUpRight className="w-5 h-5 text-yellow-400 drop-shadow-lg" />;
      case "repay_collateral":
        return <ArrowDownLeft className="w-5 h-5 text-red-400 drop-shadow-lg" />;
      default:
  ...
  const openBlockExplorer = (hash: string) => {
    const explorerUrl = `https://scan.test2.btcs.network/tx/${hash}`;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-3 text-gray-300 group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs text-gray-500 uppercase tracking-wider">Block</div>
                      <div className="text-white font-mono text-xs sm:text-sm truncate">
