...
const WarningMessage: React.FC<WarningMessageProps> = ({
  show,
  message = "You need to create a  first by supplying collateral and borrowing assets. Visit the Borrow page to get started.",
  actionText = "Borrow",
  onAction,
}) => {
  if (!show) return null;
        ...
        {message.split(actionText).map((part, index, array) => (
          <React.Fragment key={index}>
            ...
            {index < array.length - 1 && (
              <button onClick={onAction} className="font-medium hover:underline cursor-pointer">
                {actionText}
              </button>
            )}
          </React.Fragment>
