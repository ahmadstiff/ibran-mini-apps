...
export const useBorrow = (
  ...
  const { maxUserBorrow, isLoadingMaxUserBorrow, refetchMaxUserBorrow } =
  ...
  const maxBorrowAmount =
    maxUserBorrow !== undefined && maxUserBorrow !== null
  ...
  const handleBorrow = async (lendingPoolAddress: HexAddress) => {
    ...
    if (maxUserBorrow !== undefined && maxUserBorrow !== null) {
      ...
      if (borrowAmount > maxBorrowFromContract) {
        toast.error(
          `Borrow amount exceeds your maximum borrow limit. Maximum available: ${maxBorrowFromContract.toFixed(2)}`,
          {
            style: {
              background: "rgba(239, 68, 68, 0.1)",
              backdropFilter: "blur(10px)",
      ...
      if (borrowAmount > maxBorrowAmount) {
        toast.error(
          `Borrow amount exceeds available limit. Maximum available: ${maxBorrowAmount.toFixed(2)}`,
          {
            style: {
              background: "rgba(239, 68, 68, 0.1)",
              backdropFilter: "blur(10px)",
