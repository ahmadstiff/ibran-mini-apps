...
export const useRepay = (
  ...
  const { totalBorrowAssets, totalBorrowAssetsLoading, refetchTotalBorrowAssets } =
  ...
  const repay = useCallback(
    async (
      ...
      try {
        await writeContract({
          ...
          args: [userShares, tokenInfo.address as `0x${string}`, false],
        });
      } catch (err) {
    ...
    [borrowToken, lendingPoolAddress, chainId, totalBorrowAssets, totalBorrowShares, writeContract]
  );
